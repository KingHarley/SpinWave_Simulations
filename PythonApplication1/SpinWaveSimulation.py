...
appliedH = 100 * 79.57747
gamma = 2 * numpy.pi * 2.996702 * 10 ** 10 * 4 * numpy.pi * 10 ** -7
ampMs = (18798/ (10 ** 4 * muZero))
gilDamping = 0.0107
exchangeA = 1.5 * 10 ** -7 * 10 ** -4
surface_Ks1 = 1.86987 * 10 ** -3
surface_Ks2 = 1.86987 * 10 ** -3
surface_Ds1 = 0
surface_Ds2 = 0
...
pinning_d1y = -1 * (2 * surface_Ks1  / (muZero * abs(satMs))) * muZero * abs(satMs) / (2 *  exchangeA ) #changed, re-added factor of 2, d1y corresponds to Hu1 now
pinning_d2y = -1 * (2 * surface_Ks2  / (muZero * abs(satMs))) * muZero * abs(satMs) / (2 *  exchangeA ) #changed, re-added factor of 2, d2y corresponds to Hu2 now 
pinning_d1x = 0 
pinning_d2x = 0 
...
def create_global_vars_matrix():
	...
	matrix[23] = ["freq_lower:", freq_lower/(2 * numpy.pi)]		#changed storing CGS units for easier viewing
...
def update_global_vars(indV):
	...
	applied_Hu1= 2 * surface_Ks1 / (muZero * abs(satMs))
	applied_Hu2= 2 * surface_Ks2 / (muZero * abs(satMs))
	pinning_d1y= -1 * (applied_Hu1) * muZero * abs(satMs) / (2 * exchangeA)		#Changed no longer dividing by factor of 2 as old formalism is believed to be correct
	pinning_d2y= -1 * (applied_Hu2) * muZero * abs(satMs) / (2 * exchangeA)		#Changed d1y corresponds to Hu1 now, d2y corresponds to Hu2 now
	pinning_d1x= 0
	pinning_d2x= 0
	bulk_DD1= 1j * surface_Ds1 / exchangeA
	bulk_DD2= -1j * surface_Ds2 / exchangeA
...
def update_dict_vars(indV, arg_dict):		# changed to using arg_dict, conforms with laptop, allows for using 'id' kwarg
	temp = copy.deepcopy(indV)
	temp.update(arg_dict)
	#for val in args:
	#	if val[0] in temp:
	#		#print(val[0])
	#		#print(val[1])
...
def Gsi(t):
	...
	integrated = integrate.quad(create_Gsi_integrand, 1, 40 / thicknessSi, args =(t), limit = 500)
	result = ((integrated[0] + Ci(40 * t / thicknessSi) / (epsZero * (1 + epsilonSi))) / numpy.pi)
	return result.real
...
def del_H(w):
	result = numpy.sign(appliedH) * 1j * w * gilDamping / gamma		#Changed depends on sign of appliedH
	return result
...
def create_JJI_A_matrix(num_signal, num_ground, num_tot, G_vecs):
	...
	A = numpy.zeros((num_tot, num_tot), dtype = complex)			#changed initialise A matrix with zeros instead of ones
	for i in range(num_signal):
...
def create_JJI_E2_vec(Gout, ww):
	...
	return -E2						#Changed (added -ve sign)
...
def JJI(H, w, Ycss):
	...
	matrix_JJI_ZZ = -numpy.linalg.inv(numpy.transpose(matrix_JJI_I)) / del_width      #Changed (added -ve sign)
	var_Y11 = Ycss * w
	#var_time = time.time()
	...
	print("Frequency: ", w/(2*numpy.pi))
	print("Time Frequency: ", time.time() - var_time)
	#print("vec_JJI_Z: ")
	#print(vec_JJI_Z)
	final_result = numpy.concatenate((vec_JJI_Z, vec_JJI_ww2))
	return final_result    #changed returning both Z vecotr as well as current densities
...
def create_savefile(filename, result_matrix, frequencies):
	matrix = numpy.zeros((len(frequencies), 50), dtype = numpy.float64)		#changed to 50 so as to store unwrapped phase as well
	...
	matrix[:,49] = numpy.unwrap(matrix[:,32])
...
def main():
	...
	sim_varying_args(directory, var_Ycss, -0 * 79.577473, -1100 * 79.577473, 20, 'appliedH', id ="Ms=18798_g=2.996702_Hu12=1250_2.464umsep")
	return 0
